<script>
	...
	async function save() {
		...
		filtered = Object.fromEntries(Object.entries(filtered).filter(([key]) => key !== 'profile'));
		filtered = Object.fromEntries(Object.entries(filtered).filter(([key]) => key !== 'view'));
		filtered = Object.fromEntries(Object.entries(filtered).filter(([key]) => key !== 'views'));
		filtered = Object.fromEntries(Object.entries(filtered).filter(([key]) => key !== 'user_id'));
