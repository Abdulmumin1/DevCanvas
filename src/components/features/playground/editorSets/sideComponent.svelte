<script>
	import { isOwner, current_data } from '$lib/stores/index.js';
	...
	import { saveData, tabsView } from '$lib/stores/playground.js';

	import { hide_css, hide_js, editorFontSize } from '$lib/config/settings.js';
	...
	>
		...
		<li class="w-full">
			...
			<ul class="flex flex-row flex-wrap gap-2 md:*:w-[370px]">
				...
				<li class="w-full rounded-lg bg-gray-200 p-2 dark:bg-primary">
					<SingleSetting label={'Tabs View'} on:checked={handleTabView} checked={$tabsView} />
				</li>
			</ul>
		</li>
