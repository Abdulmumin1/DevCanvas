<script>
   ...
   <div
	 class="w-full flex  flex-col rounded-xl bg-neutral-50 p-2  transition-all duration-300 hover:shadow-lg dark:bg-secondary-dark aspect-video"
   ...
   >
	 <div class="relative overflow-hidden rounded-xl">
	   ...
	   <div class="absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-black/50 to-transparent opacity-0 transition-opacity duration-300" class:opacity-100={isHovered}></div>
	 </div>
	 ...
	 <div class="flex flex-1 flex-col justify-between p-3">
	   ...
	   <div class="mt-3 flex w-full items-center justify-between">
		 ...
		 {:else if profile}
		   <div in:fade class="flex items-center gap-2">
			 <div class="h-6 w-6 overflow-hidden rounded-full bg-sky-500 dark:bg-sly-400"></div>
			 <a class="text-sm font-medium capitalize hover:underline" href={`/${profile}`}>{profile}</a>
		   </div>
		 {:else}
		   <div in:fade class="flex items-center gap-2">
